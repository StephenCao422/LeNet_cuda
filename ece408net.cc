...
std::string get_data_dir() {
  std::string path =
      get_env_or_default("LENET_DATA_DIR", "/projects/bche/project/data/fmnist-86/");
...
std::string get_weights_path() {
  return get_env_or_default("LENET_WEIGHTS_PATH", "weights-86.bin");
}
...
Network createNetwork_CPU(bool customCPUConv)
{
  ...
  //load weights
  dnn.load_parameters(get_weights_path());
