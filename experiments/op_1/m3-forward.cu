...
{
    /*
    The unfused approach:
      1. Unroll input  → unrolled_matrix  [Height_unrolled × Width_unrolled]
      2. matmul: mask [Map_out × Height_unrolled] × unrolled_matrix → C [Map_out × Width_unrolled]
      3. Permute C: [Map_out][Batch][H_out][W_out] → output[Batch][Map_out][H_out][W_out]

    Grid  : (ceil(Width_unrolled/TILE_WIDTH), ceil(Map_out/TILE_WIDTH), 1)
    Block : (TILE_WIDTH, TILE_WIDTH, 1)
    Thread (ty, tx) computes output element at (row=m, col=b*H_out*W_out+h*W_out+w)
    */
    ...
    for (int tileId = 0; tileId < numTiles; tileId++) {
        ...
        int a_col = tileId * TILE_WIDTH + tx; // column in mask matrix
        ...
        int b_row = tileId * TILE_WIDTH + ty; // row index in unrolled matrix (0..Height_unrolled-1)
    ...
    // matmul output layout:  [m][b * H_out * W_out + h_out * W_out + w_out]  (Map_out × Width_unrolled)
    // desired output layout:  output[b][m][h_out][w_out]  (Batch × Map_out × H_out × W_out)
    if (row < Map_out && col < Width_unrolled) {
...
__host__ void GPUInterface::conv_forward_gpu(float * __restrict__  device_output, const float * __restrict__  device_input, const float * __restrict__  device_mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    dim3 grid((unsigned int)((Width_unrolled + TILE_WIDTH - 1) / TILE_WIDTH),
              (unsigned int)((Map_out        + TILE_WIDTH - 1) / TILE_WIDTH),
              1);
...
__host__ void GPUInterface::conv_forward_gpu_epilog(float * __restrict__  host_output, float * __restrict__  device_output, float * __restrict__  device_input, float * __restrict__  device_mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    cudaMemcpy(host_output, device_output,
               (size_t)Batch * Map_out * Height_out * Width_out * sizeof(float),
