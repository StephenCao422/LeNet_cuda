...
template<int MAP_OUT, int CHANNEL, int k_size, int HEIGHT, int WIDTH, int ROWS_PER_BLOCK>
__global__ void direct_conv_kernel(const float * __restrict__ input, float * __restrict__ output, int Batch)
{
    ...
    const int NTHREADS = ROWS_PER_BLOCK * WIDTH_OUT;
    ...
    //load input strip to shared mem
    ...
    for (int c = 0; c < CHANNEL; c++) {
        ...
        for (int p = 0; p < k_size; p++) {
            ...
            for (int q = 0; q < k_size; q++) {
                ...
                #pragma unroll
                for (int m = 0; m < MAP_OUT; m++) {
                    acc[m] += const_mask[((m * CHANNEL + c) * k_size + p) * k_size + q] * v;
                }
            }
        }
    }
...
__global__ void wmma_strip_conv_batch2_kernel(const float * __restrict__ input, float * __restrict__ output, int Batch)
{
    ...
    auto write_c2 = [&](size_t img_out_base, int m, int bcol_pair, float v0, float v1) {
        if (bcol_pair >= BN) return;
        int h_out_local = bcol_pair / WIDTH_OUT;
        int w_out = bcol_pair - h_out_local * WIDTH_OUT;
        int h_out = h_row_base + h_out_local;
        if (h_out >= HEIGHT_OUT) return;
    ...
    write_c2(img0_out_base, groupId, col_lo, c_frag0.x[0], c_frag0.x[1]);
    write_c2(img0_out_base, groupId + 8, col_lo, c_frag0.x[2], c_frag0.x[3]);
    write_c2(img0_out_base, groupId, col_hi, c_frag0.x[4], c_frag0.x[5]);
    write_c2(img0_out_base, groupId + 8, col_hi, c_frag0.x[6], c_frag0.x[7]);

    if (b_pair + 1 < Batch) {
        const size_t img1_out_base = (size_t)(b_pair + 1) * (MAP_OUT * IMAGE_SIZE);
        write_c2(img1_out_base, groupId, col_lo, c_frag1.x[0], c_frag1.x[1]);
        write_c2(img1_out_base, groupId + 8, col_lo, c_frag1.x[2], c_frag1.x[3]);
        write_c2(img1_out_base, groupId, col_hi, c_frag1.x[4], c_frag1.x[5]);
        write_c2(img1_out_base, groupId + 8, col_hi, c_frag1.x[6], c_frag1.x[7]);
    }
}
